...
int main() {
    const std::string filePath = 
        "data/wikipedia-small/benchmark-dev-wikipedia-bge-m3-small.h5"; //Ejecutar primero download_data y mirar el path donde se han descargado los datos!
    ...
    const int K   = 15;
    auto t0  = clk::now();
    auto gt2  = brute_force_mip(data, num_vectors, queries, n_queries, dim, K);
    printf("  exact k-NN:  %ld ms\n\n", ms_since(t0));
    printf("  brute force mip recall@%d: %.4f\n\n", K, pipnn::recall_at_k(gt2, gt, K, n_queries));
         ...
         printf("── opt) pipnn::IndexDot ──\n");
    {
        ...
        printf("  graph: avg_deg=%.1f  max_deg=%.0f  bidir=%.1f%%\n",
               s.avg_deg, 0.0, 100.0*s.frac_bidir);
